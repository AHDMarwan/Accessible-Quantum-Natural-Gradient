...
The supervised wrapper adds one input-encoding stage. For U(1), the encoding
is number conserving. Optimizer mathematics is unchanged:
G_acc = mean_b J_b^T Sigma_b^+ J_b; Full-QNG = exact QFIM = 4*g_FS;
(G+lambda I)d=grad L; theta <- theta-lr*d.

Implementation optimizations only: native input broadcasting, cached QNodes and
metric transforms, Lightning adjoint VJPs for loss/AQNG, vectorized covariance,
and identical metric caching schedules for AQNG and Full-QNG.
"""
...
def _binary_dataset(name):
    ...
    if name == "breast_cancer":
        ...
        return ds.data.astype(float), ds.target.astype(int)
    if name == "wine01":
...
def make_data(name, seed, n_samples, n_qubits):
    X, y = _binary_dataset(name)
    if n_samples > len(X):
        raise ValueError(f"n_samples={n_samples} exceeds {name} size={len(X)}")
    X, _, y, _ = train_test_split(X, y, train_size=n_samples,
                                   random_state=1729, stratify=y)
    ...
    return dict(X_train=Xtr, X_test=Xte, y_train=ytr, y_test=yte,
                y_train_pm=2*ytr-1, y_test_pm=2*yte-1)

def brickwork_pairs(n, layer):
    return [(w,w+1) for w in range(layer % 2, n-1, 2)]

def u1_xy_pairs(n):
    return ([(w,w+1) for w in range(0,n-1,2)] +
            [(w,w+1) for w in range(1,n-1,2)])
...
def make_haar_blocks(seed, n, layers):
    rng = onp.random.default_rng(91771 + int(seed)); blocks = {}
    for layer in range(layers):
        for pair in brickwork_pairs(n, layer): blocks[(layer,pair)] = haar_unitary4(rng)
    return blocks
...
class CircuitBundle:
    def __init__(self,family,cfg):
        ...
        dev_fast=qml.device("lightning.qubit",wires=cfg.n_qubits,shots=None,batch_obs=True)
        ...
        dev_qng=qml.device("default.qubit",wires=cfg.n_qubits,shots=None)
    ...
    def aqng_metric(self,theta,Xm):
        ...
        vals,vecs=onp.linalg.eigh(S); vals=onp.maximum(vals,0.0)
        scale=onp.maximum(onp.max(vals,axis=1),1.0); tol=self.cfg.rcond*scale[:,None]
        inv=onp.where(vals>tol,1.0/onp.maximum(vals,tol),0.0)
        Sinv=onp.einsum("bik,bk,bjk->bij",vecs,inv,vecs,optimize=True)
        G=onp.einsum("bri,brs,bsj->ij",J,Sinv,J,optimize=True)/B
        return 0.5*(G+G.T)
    def full_qng_metric(self,theta,Xm):
        ...
        G *= 4.0/float(len(Xm)); return 0.5*(G+G.T)
...
def spectral(G,lam,rcond):
    ...
    cond=float(pos[-1]/pos[0]) if len(pos)>1 else (1.0 if len(pos)==1 else onp.inf)
    reg=eig+lam
    return dict(metric_rank=int(rank),metric_trace=float(onp.trace(G)),metric_condition=cond,
                regularized_metric_condition=float(reg[-1]/reg[0]),metric_min_eig=float(eig[0]),metric_max_eig=mx)
...
def train_method(bundle,method,cfg,data,batches):
    ...
    G=None; builds=0; mt=gt=st=0.0; firstG=firstg=firstd=None; mdiags=[]; curves=[]; ttot=time.perf_counter()
    for step,ids in enumerate(batches,1):
        mids=ids[:min(cfg.metric_batch,len(ids))]; refresh=G is None or ((step-1)%cfg.metric_every==0)
        if refresh:
            t=time.perf_counter(); G=bundle.aqng_metric(theta,X[mids]) if method=="AQNG" else bundle.full_qng_metric(theta,X[mids]); mt+=time.perf_counter()-t; builds+=1
        ...
        t=time.perf_counter(); g=tonp(grad_fn(theta)).reshape(-1); gt+=time.perf_counter()-t; loss=float(grad_fn.forward)
        ...
        curves.append(dict(step=step,loss_before=loss,gradient_norm=float(onp.linalg.norm(g)),direction_norm=float(onp.linalg.norm(direction)),metric_refreshed=int(refresh)))
    ...
    result=dict(dataset=cfg.dataset,family=bundle.family,method=method,seed=cfg.seed,n_qubits=cfg.n_qubits,n_layers=cfg.n_layers,n_params=bundle.p,n_samples=cfg.n_samples,steps=cfg.steps,lr=cfg.lr,lam=cfg.lam,loss_batch=cfg.loss_batch,metric_batch=cfg.metric_batch,metric_every=cfg.metric_every,readout="Z<=2",train_loss=tr["loss"],train_acc=tr["acc"],test_loss=te["loss"],test_acc=te["acc"],total_seconds=total,metric_seconds=mt,gradient_seconds=gt,solve_seconds=st,metric_builds=builds)
    md=pd.DataFrame(mdiags); result.update(metric_rank_first=int(md.iloc[0].metric_rank),metric_trace_first=float(md.iloc[0].metric_trace),metric_condition_first=float(md.iloc[0].metric_condition),regularized_metric_condition_first=float(md.iloc[0].regularized_metric_condition),metric_rank_last=int(md.iloc[-1].metric_rank),metric_trace_last=float(md.iloc[-1].metric_trace))
    return dict(result=result,curves=curves,metric_diags=mdiags,first_metric=firstG,first_grad=firstg,first_direction=firstd)
...
def run_job(cfg,output_dir):
    ...
    output_dir.mkdir(parents=True,exist_ok=True); data=make_data(cfg.dataset,cfg.seed,cfg.n_samples,cfg.n_qubits); batches=schedule(len(data["X_train"]),cfg)
    ...
    for family in tqdm(FAMILIES,desc=f"{cfg.dataset} seed={cfg.seed}"):
        ...
        pairs.append(dict(dataset=cfg.dataset,family=family,seed=cfg.seed,initial_direction_cosine=cosine(a["first_direction"],q["first_direction"]),initial_gradient_cosine=cosine(a["first_grad"],q["first_grad"]),trace_ratio_Gacc_over_Gq=float(onp.trace(Ga)/onp.trace(Gq)) if onp.trace(Gq)!=0 else onp.nan,max_eig_Gacc_minus_Gq=float(onp.linalg.eigvalsh(diff)[-1])))
    ...
    meta=dict(config=cfg.__dict__,families=list(FAMILIES),methods=list(METHODS),readout="all diagonal Pauli-Z strings through weight 2",pennylane=qml.__version__,numpy=onp.__version__,python=platform.python_version(),github_sha=os.environ.get("GITHUB_SHA"),github_run_id=os.environ.get("GITHUB_RUN_ID"))
